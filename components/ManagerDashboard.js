...
export default function ManagerDashboard({ onBack, onOpenEmployee }) {
  ...
  const [mergeRecs, setMergeRecs] = useState({})   // { 지점명: [그 지점 합산대상 월 레코드] } — 폐업 등 여러 달 합산 표시용
  ...
  useEffect(() => {
    const brs = Object.keys(MERGE_BRANCHES)
    ...
    ;(async () => {
      ...
      for (const b of brs) {
        const { data } = await supabase.from('payroll').select('*')
          .eq('branch', b).eq('year', year).in('month', MERGE_BRANCHES[b])
        out[b] = (data || []).filter(r => !r.deleted_at)
      }
      if (!cancelled) setMergeRecs(out)
    })()
    return () => { cancelled = true }
  }, [year, month, branch])

  // ── 지점별 월별: 그 해 전체(12개월) 레코드 로드 ──
  useEffect(() => {
  ...
  // 합산 지점(폐업 등): 여러 달을 '이름' 기준으로 한 사람으로 묶어 유닛 생성 (계좌는 있는 값 사용)
  function combinedUnitsForMergeBranch(b) {
    const list = (mergeRecs[b] || []).filter(r => !isRecordOnly(r))
    const byName = {}
    for (const r of list) {
      if (!byName[r.emp_name]) byName[r.emp_name] = { key: 'mg:' + b + ':' + r.emp_name, recs: [], account: r.account_number || '' }
      byName[r.emp_name].recs.push(r)
      if (!byName[r.emp_name].account && r.account_number) byName[r.emp_name].account = r.account_number
    }
  ...
  function downloadTransferXlsx(kind = 'all') {
    ...
    groups.forEach((g, gi) => {
      ...
      // 지점 제목 줄 (5칸 가로 병합)
      put(r, 0, `${g.branch}   (${g.count}명 · ${g.total.toLocaleString()}원)`, {
        font: { sz: 13, bold: true, color: { rgb: 'FFFFFF' } },
        alignment: { horizontal: 'left', vertical: 'center' },
        fill: { fgColor: { rgb: '6B6357' } }, border: allBd,
      })
      for (let c = 1; c < COLS; c++) put(r, c, '', { fill: { fgColor: { rgb: '6B6357' } }, border: allBd })
      merges.push({ s: { r, c: 0 }, e: { r, c: COLS - 1 } })
      rowH[r] = 26; r++
      // 칸 제목(헤더) 줄
      row5(r, ['구분', '이름', '금액', '은행', '계좌번호'], c => ({
        font: { sz: 11, bold: true, color: { rgb: '5A5348' } },
        alignment: { horizontal: (c === 2) ? 'right' : (c === 0 || c === 3) ? 'center' : 'left', vertical: 'center' },
        fill: { fgColor: { rgb: 'EDE5D2' } }, border: allBd,
      }))
      rowH[r] = 22; r++
      // 사람 줄
      g.sorted.forEach(u => {
        ...
        put(r, 0, unitIsAlba(u) ? 'pt' : '직원', { fill, alignment: { horizontal: 'center', vertical: 'center' }, font: { sz: 11, bold: true, color: { rgb: unitIsAlba(u) ? 'A06D12' : '4A5663' } }, border: allBd })
        put(r, 1, unitNames(u), { fill, font: { sz: 13 }, alignment: mid, border: allBd })
        put(r, 2, unitAmt(u), { fill, numFmt: '#,##0', alignment: { horizontal: 'right', vertical: 'center' }, font: { sz: 14, bold: true }, border: allBd })
        put(r, 3, a.bank, { fill, font: { sz: 11 }, alignment: { horizontal: 'center', vertical: 'center' }, border: allBd })
        put(r, 4, a.num, { fill, font: { sz: 13 }, alignment: mid, border: allBd })
        rowH[r] = 26; r++
      })
      // 합계 줄
      const tf = { fgColor: { rgb: 'F0EAD9' } }
      put(r, 0, '', { fill: tf, border: allBd })
      put(r, 1, '합계', { fill: tf, font: { sz: 13, bold: true }, alignment: { vertical: 'center' }, border: allBd })
      put(r, 2, g.total, { fill: tf, numFmt: '#,##0', alignment: { horizontal: 'right', vertical: 'center' }, font: { sz: 14, bold: true }, border: allBd })
  ...
  const css = `
    ...
    .md-hero-label { font-size: 11px; letter-spacing: 0.1em; color: #999; margin-bottom: 14px; }

    /* 공제 전 금액 → (공제) → 실지급액 두 갈래 카드 (전 지점·단일 지점 공통) */
    .md-money { display: flex; align-items: stretch; gap: 10px; }
    .md-money-card { flex: 1; min-width: 0; background: #faf8f3; border: 1px solid #efeadd; border-radius: 12px; padding: 16px 18px; }
    .md-money-card.primary { background: #fbf6ea; border-color: #ecdcb0; box-shadow: 0 2px 10px rgba(184,149,74,0.1); }
    .md-money-k { font-size: 12.5px; color: #7a7468; font-weight: 700; line-height: 1.35; }
    .md-money-k span { display: block; font-size: 10.5px; color: #aaa; font-weight: 500; margin-top: 1px; }
    ...
    @media (max-width: 640px) {
      ...
      .md-money-mid { flex-direction: row; gap: 10px; align-items: baseline; padding: 6px 0; }
    }

    /* 단일 지점: 인원·마감 간단 메타 줄 */
    .md-meta-line { display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 14px; font-size: 13px; color: #777; }
    .md-meta-line b { color: #1a1a1a; font-weight: 700; }
    .md-meta-line em { font-style: normal; color: #aaa; }
        ...
        {loading ? (
            ...
            <div className="md-hero">
              ...
              <div className="md-money">
                ...
                <div className="md-money-card primary">
                  ...
                  <div className="md-money-v gold">{fmt(netAll)}<small>원</small></div>
                </div>
              </div>
              <div className="md-split">
                <span><b className="dot staff" />직원 공제 전 {fmt(staffAll)} · 실지급 {fmt(staffNetAll)}</span>
                <span><b className="dot alba" />알바 공제 전 {fmt(albaAll)} · 실지급 {fmt(albaNetAll)}</span>
              </div>
            </div>
          ...
          <>
            ...
            <div className="md-meta-line">
              <span>👤 인원 <b>{records.length}명</b> <em>(직원 {curStaff}·알바 {curAlba})</em></span>
              <span>✓ 마감 <b>{curFinal}/{records.length}명</b></span>
            </div>
            ...
            {records.length > 0 && (
              <>
                ...
                <div className="md-money">
                  ...
                  <div className="md-money-card primary">
                    <div className="md-money-k">실지급액<span>실제 통장 출금액</span></div>
                    <div className="md-money-v gold">{fmt(curStaffNet + curAlbaNet)}<small>원</small></div>
                  </div>
                </div>
                ...
                <div className="md-split-row">
                  <div className="md-split-card">
                    ...
                    <div className="md-split-line"><span className="md-split-k">공제 전</span><span className="md-split-v">{fmt(curStaffTotal)}원</span></div>
                  ...
                  <div className="md-split-card">
                    ...
                    <div className="md-split-line"><span className="md-split-k">공제 전</span><span className="md-split-v">{fmt(curAlbaTotal)}원</span></div>
